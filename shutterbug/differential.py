...
def calculate_differential_magnitudes(
    data: pd.DataFrame, target_star: str, reference_stars: List[str]
) -> pd.DataFrame:
    ...
    merged = pd.merge(
        target_data, reference_data, on="JD", suffixes=("_target", "_ref")
    )
    ...
    merged["differential_error"] = np.sqrt(
        merged["Error_target"] ** 2 + merged["Error_ref"] ** 2
    )
    # Average over reference stars
    merged = (
        merged.groupby(["Name_target", "JD"], observed=True)
        .agg(
            {
    ...
    result = merged[
        ["Name_target", "JD", "differential_magnitude", "differential_error"]
    ]
...
def find_reference_stars(
    ...
    logging.info(
        f"Found {len(candidates)} reference stars for target {target_star}, maximum distance: {candidates['distance'].max():.2f} pixels"
    )
